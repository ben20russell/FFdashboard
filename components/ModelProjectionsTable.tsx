      ...
      <div className="mt-4 overflow-x-auto" data-testid="projections-table-wrapper">
        <table className="min-w-full divide-y divide-dashboard-border" data-testid="projections-table">
          <thead className="bg-slate-50">
            <tr>
              <th className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-dashboard-muted">Rank</th>
              ...
              <th className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide text-dashboard-muted">Injury</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-dashboard-border">
            {sortedPlayers.length === 0 ? (
              <tr data-testid="projection-empty-state">
                <td colSpan={6} className="px-3 py-5 text-sm text-dashboard-muted">
              ...
              sortedPlayers.map((player, index) => (
                <tr key={player.id} data-testid={`projection-row-${index + 1}`}>
                  <td className="px-3 py-2 text-sm text-dashboard-muted">{player.overallRank ?? '-'}</td>
                  ...
                  <td className="px-3 py-2 text-sm text-dashboard-muted">{player.injuryStatus ?? 'Healthy'}</td>
                </tr>
              ))
            )}
          </tbody>
